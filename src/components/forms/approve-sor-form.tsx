...
import {groupAndAggregateItems, selectItemsByQuantity} from "functions/utils";
  ...
  return (
    <div className="flex items-center mb-2">
      ...
      <InputWrapper outerClassName="sm:col-span-6 w-full">
        ...
        <div>
          {currentItem?.description?.length > 40
            ? `${currentItem?.description?.substr(0, 40)}...`
...
const Index: React.FC<IAddUserFormProps> = ({closeModal, sor}) => {
  const defaultValues = [
    {
      itemId: null,
      quantity: null,
  ...
  const formItems = sor.requestItems.map((it: any) => ({
    itemId: it.itemTypeId.itemType,
  ...
  const itemIds = sor.requestItems.map((reqItems) => reqItems.itemTypeId.id);
  const params = {
    itemTypeId: itemIds.join(","),
    sold: false,
    locked: false,
  };
  const {data: requestedItemsFromLocation, isLoading} = useFetchData<
    IInventoryItem[]
  ...
  const onSubmit = async (data: FormProps) => {
    const items = formSectionValues.map((it: any) => ({
      itemName: it.itemId,
      quantity: it.quantity,
    }));

    data.items = selectItemsByQuantity(items, requestedItemsFromLocation);

    data.stockOutRequestOrderId = sor.stockOutRequestOrderId;
    data.toLocation = sor.requestBy.name;
    setLoading(true);
    try {
      await createStockOut(data);
      setLoading(false);
      mutate(`stock-out-requests/${sor.stockOutRequestOrderId}`);
  ...
  useEffect(() => {
    const grouped = groupAndAggregateItems(requestedItemsFromLocation);
    setInventoryItems(grouped);
  }, [requestedItemsFromLocation]);

  return (
    <FormProvider {...methods}>
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 p-4">
        <div className="space-y-2">
          <div className="grid grid-cols-1 gap-y-2 gap-x-2 sm:grid-cols-12 mb-4">
            <InputWrapper outerClassName="sm:col-span-6">
              <Label id="phone">Tax office</Label>
              <div>{sor?.requestBy?.value}</div>
            </InputWrapper>
            <InputWrapper outerClassName="sm:col-span-6">
          ...
          <div>
            <p className="border-b mb-6">Requested Inventory Items</p>
          </div>
