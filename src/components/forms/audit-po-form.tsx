...
import {auditProcurement} from "slices/actions/procurementActions";
...
const Index: React.FC<IAuditPOFormProps> = ({
  ...
  const onSubmit = async (payload: FormProps) => {
    try {
      setLoading(true);
      const {data} = await auditProcurement({...payload}, po.orderId);
      toast.success("Purchasing Order has been Audited successfully!");
      updatePoState(data);
      closeModal();
    } catch (error) {
      handleApiError(
