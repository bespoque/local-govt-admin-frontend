...
import {auditStockOut} from "slices/actions/stockOutActions";

type FormProps = {
  auditComments: string;
};

interface IAuditSOFormProps {
  closeModal: () => void;
  so: IStockOut;
  updateSoState?: (so: IStockOut) => void;
}

const Index: React.FC<IAuditSOFormProps> = ({
  so,
  closeModal,
  updateSoState,
}) => {
  ...
  const onSubmit = async (payload: FormProps) => {
    try {
      setLoading(true);
      const {data} = await auditStockOut({...payload}, so.stockOutOrderId);
      toast.success("Stock out has been Audited successfully!");
      updateSoState(data);
      closeModal();
    } catch (error) {
      handleApiError(
