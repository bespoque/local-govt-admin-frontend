...
import {updateStockOutRequest} from "slices/actions/stockOutActions";
...
const Index: React.FC<IAddUserFormProps> = ({closeModal, sor}) => {
  ...
  const onSubmit = async (data: FormProps) => {
    ...
    setLoading(true);
    try {
      await updateStockOutRequest(data, sor.stockOutRequestOrderId);
      setLoading(false);
  ...
  useEffect(() => {
    fetchInventoryItems({
      ...
      const grouped = groupAndAggregateItems(res.data);
      setInventoryItems(grouped);
    });
  }, []);

  const handleTaxOfficeChange = async (taxOffice) => {
    setFormSectionValues(defaultValues as any);
