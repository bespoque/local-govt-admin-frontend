...
const Index: React.FC = () => {
  ...
  const {data, isLoading: loading} = useFetchData<IVehicleLicenceRequest>(
    `/vehicle-licence-requests/${newLicenseId}`
  );
  ...
  const createAssessmentHandler = async () => {
    ...
    try {
      setReqLoading(true);
      await printVlrAssessment(newLicenseId);
      mutate(`/vehicle-licence-requests/${newLicenseId}`);
  ...
  const printTemplateHandler = async (template: PRINT_TEMPLATE) => {
    ...
    try {
      setReqLoading(true);
      // await sendToPrinterHandler(newLicenseId);
