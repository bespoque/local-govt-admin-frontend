...
import UploadDocumentsForm from "components/forms/upload-vl-docs-form";
...
import {VCofOActionButtons} from "components/vehicle-license/vehicleCofORequestActions";

import {
  approveChangeOfOwnership,
  printChangeOfOwnershipAssessment,
  sendCofOToPrinterHandler,
  verifyChangeOfOwnership,
...
const Index: React.FC = () => {
  ...
  const newLicenseId = router?.query?.newLicenseId as string;
  const {data, isLoading: loading} = useFetchData<IVehicleLicenceRequest>(
    `/change-ownerships/${newLicenseId}`
  );

  // The IVehicleChangeOfOwnershipRequest to replace the IVehicleLicenceRequest above

  // const {data, isLoading: loading} = useFetchData<IVehicleChangeOfOwnershipRequest>(
  //   `/change-ownerships/${newLicenseId}`
  // );
  ...
  // I'm guessing bug is arising as a result of vehicle not being an array
  ...
  const createAssessmentHandler = async () => {
    ...
    try {
      setReqLoading(true);
      await printChangeOfOwnershipAssessment(newLicenseId);
      mutate(`/change-ownerships/${newLicenseId}`);
  ...
  const verifyHandler = async () => {
    ...
    try {
      setReqLoading(true);
      await verifyChangeOfOwnership(newLicenseId);
      mutate(`/change-ownerships/${newLicenseId}`);
  ...
  const approveHandler = async () => {
    ...
    try {
      setReqLoading(true);
      await approveChangeOfOwnership(newLicenseId);
      mutate(`/change-ownerships/${newLicenseId}`);
      toast.success("Request has been Verified Successfully!");
  ...
  const printTemplateHandler = async (template: PRINT_TEMPLATE) => {
    ...
    try {
      setReqLoading(true);
      await sendCofOToPrinterHandler(newLicenseId);
  ...
  if (isPrinting) {
    switch (printTemplate) {
      ...
      //   return <A4PrintOut v={currentVehicle} />;
